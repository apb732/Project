    ...
    output = {
        "wlan.tag": {
            ...
            "wlan.vht.mcsset": {
                ...
                "wlan.vht.mcsset.vht_ext_nss_bw_capable": str((tx_mcs_map >> 15) & 0x1),
...
def parse_he_capabilities(he_body: bytes) -> dict:
    ...
    # =====================================================
    # PPE THRESHOLDS (if present)
    # =====================================================

    # Check PPE Present flag (Bit 55 of PHY)
    if bits(55, 1) == 1:

        ppe_offset = 17 + 4  # 6 MAC + 11 PHY + 4 MCS
        ppe_bytes = he_body[ppe_offset:]

        if len(ppe_bytes) == 0:
            return result

        ppe_val = int.from_bytes(ppe_bytes, "little")

        def ppe_bits(start, length):
            return (ppe_val >> start) & ((1 << length) - 1)
        ...
        # -------------------------------------------------
        nss = ppe_bits(0, 3)
        ru_mask = ppe_bits(3, 4)

        ppe_tree["wlan.ext_tag.he_ppe_thresholds.nss"] = str(nss)
        ppe_tree["wlan.ext_tag.he_ppe_thresholds.ru_index_bitmask"] = f"0x{ru_mask:02x}"

        # RU mapping (Wireshark order)
        ru_types = ["242", "484", "996", "2x996"]

        bit_cursor = 7  # first 7 bits consumed

        for ss in range(nss + 1):

            ss_tree = {}

            for ru_index in range(4):

                if (ru_mask >> ru_index) & 1:
                    ru_name = f"RU allocation: {ru_types[ru_index]}"

                    ppet16 = ppe_bits(bit_cursor, 4)
                    bit_cursor += 4

                    ppet8 = ppe_bits(bit_cursor, 3)
                    bit_cursor += 3

                    ss_tree[ru_name] = {
                        "wlan.ext_tag.he_ppe_thresholds.ppet16": f"0x{ppet16:02x}",
                        "wlan.ext_tag.he_ppe_thresholds.ppet8": f"0x{ppet8:02x}",
                    }

            ppe_tree[f"NSS {ss}"] = ss_tree

            result["PPE Thresholds"] = ppe_tree
